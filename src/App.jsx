...
export default function App() {
  ...
  const filteredBottles = useMemo(() => {
    ...
    return result;
  }, [bottles, searchQuery, filterLocation, sortBy]);
  ...
  return (
    <div className="min-h-screen">
      <header className="bg-stone-900 text-stone-50 relative overflow-hidden pt-safe">
        <div className="absolute inset-0 opacity-[0.07]" style={{
          backgroundImage: 'radial-gradient(circle at 20% 30%, #f59e0b 0%, transparent 50%), radial-gradient(circle at 80% 70%, #b45309 0%, transparent 50%)'
        }} />

        <button
          onClick={() => setShowMenu(true)}
          aria-label="Menu"
          className="absolute z-10 p-2 text-stone-400 hover:text-stone-100 active:bg-stone-800/80 rounded-md transition-colors"
          style={{ top: 'calc(env(safe-area-inset-top) + 0.5rem)', right: '0.75rem' }}
        >
        ...
        <div className="max-w-5xl mx-auto px-6 py-10 relative">
          <div className="flex items-end justify-between gap-6">
            <div>
              <div className="text-[10px] uppercase tracking-[0.35em] text-amber-500/90 mb-3 font-medium">Private Cellar</div>
              <h1 className="font-display text-4xl md:text-5xl tracking-tight font-medium">Danny&rsquo;s Collection</h1>
              <p className="text-stone-400 mt-2 text-sm num">{pluralize(bottles.length, 'bottle', 'bottles')}</p>
            </div>
            <div className="hidden sm:block text-right">
              <div className="font-display text-5xl num text-amber-500 leading-none">{bottles.length}</div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-stone-500 mt-2">bottles</div>
            </div>
          </div>
        </div>
      </header>

      <div className="sticky top-0 z-20 border-b border-stone-300/70" style={{ backgroundColor: 'rgba(244, 239, 230, 0.95)', backdropFilter: 'blur(8px)' }}>
        <div className="max-w-5xl mx-auto px-6 py-3.5 flex gap-2.5 items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2.5 bg-white border border-stone-300 rounded-md text-[15px] text-stone-900 placeholder-stone-400 focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/20 transition"
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700" aria-label="Clear search">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6 pb-3 flex flex-wrap gap-2 items-center text-sm">
          {knownLocations.length > 0 && (
            <select value={filterLocation} onChange={e => setFilterLocation(e.target.value)} className="px-3 py-1.5 bg-white border border-stone-300 rounded-md text-stone-700 text-sm focus:outline-none focus:border-amber-700 max-w-[240px]">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-stone-500 text-xs uppercase tracking-wider">Sort</span>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-2 py-1.5 bg-transparent border border-stone-300 rounded-md text-stone-700 text-sm focus:outline-none focus:border-amber-700">
...
function EmptyState({ onAdd }) {
  return (
    <div className="text-center py-24">
      ...
      <h2 className="font-display text-2xl text-stone-900 mb-2">Your collection is empty</h2>
...
function BottleRow({ bottle, onEdit }) {
  ...
  if (bottle.finishedAt) {
    subParts.push({ key: 'finished', text: `Finished ${formatDate(bottle.finishedAt)}`, cls: 'text-red-700 font-bold' });
  }
  if (memoOneLine) {
    subParts.push({ key: 'memo', text: memoOneLine, cls: '' });
  }
  ...
  return (
    <li>
      <button
        onClick={onEdit}
        className="w-full text-left px-5 py-4 hover:bg-stone-50 active:bg-stone-100 transition-colors flex items-start gap-4 group"
      >
        <div className="flex-shrink-0 w-1 self-stretch rounded-full bg-stone-200 group-hover:bg-amber-700 transition-colors" />
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-3">
            <h3 className={`font-display text-[17px] leading-tight truncate font-semibold ${
            ...
            {bottle.location && (
              <div className={`flex-shrink-0 flex items-center gap-1 text-xs ${
                ...
                <span className="max-w-[140px] sm:max-w-[200px] truncate">{bottle.location}</span>
              </div>
            )}
          </div>
          {subParts.length > 0 && (
            <div className="text-[13px] text-stone-600 mt-0.5 truncate">
              {subParts.map((part, i) => (
                <span key={part.key}>
  ...
  return (
    ...
    >
      <form
        ...
        className="w-full sm:max-w-xl max-h-[94vh] flex flex-col rounded-t-2xl sm:rounded-xl border border-stone-200 shadow-2xl"
        style={{ backgroundColor: '#f4efe6' }}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200 flex-shrink-0">
          <h2 className="font-display text-xl text-stone-900 font-medium">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-4 flex-1">
          ...
          <Field label="Name" required hint="Include distillery, age, vintage, edition — anything relevant — all in one line.">
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-white border border-stone-300 rounded-md text-stone-900 focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/20"
              autoFocus
            />
          </Field>
          ...
          <Field label="Notes">
            <textarea
              ...
              className="w-full px-3.5 py-2.5 bg-white border border-stone-300 rounded-md text-stone-900 focus:outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-700/20 resize-none"
            />
          </Field>
        ...
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-stone-200 flex-shrink-0 flex-wrap pb-safe" style={{ backgroundColor: 'rgba(255,255,255,0.4)' }}>
...
function CalendarPopup({ triggerRef, value, onChange, onClear, onClose }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute bottom-full left-0 mb-2 bg-white rounded-lg shadow-xl border border-stone-200 p-3.5 w-72 z-30"
    >
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={prev} className="p-1.5 hover:bg-stone-100 rounded-md text-stone-700" aria-label="Previous month">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <div className="font-display text-[15px] font-medium text-stone-900">
          {MONTHS[viewMonth]} {viewYear}
        </div>
        <button type="button" onClick={next} className="p-1.5 hover:bg-stone-100 rounded-md text-stone-700" aria-label="Next month">
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {DAYS_OF_WEEK.map((d, i) => (
          <div key={i} className="text-center text-[10px] text-stone-500 uppercase tracking-wider font-semibold py-1">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between items-center mt-3 pt-3 border-t border-stone-200">
        <button type="button" onClick={pickToday} className="text-xs text-amber-700 hover:text-amber-800 font-medium">
          Today
        </button>
        {value && (
          <button type="button" onClick={onClear} className="text-xs text-stone-500 hover:text-red-700 font-medium">
            Clear
          </button>
        )}
      </div>
...
function BackupModal({ bottles, onImport, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200 flex-shrink-0">
          <h2 className="font-display text-xl text-stone-900 font-medium">Backup &amp; Restore</h2>
          <button type="button" onClick={onClose} className="text-stone-400 hover:text-stone-700 p-1" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 py-3 border-t border-stone-200 text-[11px] text-stone-500 leading-relaxed">
