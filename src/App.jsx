  ...
  return (
    <div className="min-h-screen">
      <header className="bg-stone-900 text-stone-50 relative overflow-hidden pt-safe">
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-6 py-10 relative">
          <div className="flex items-end justify-between gap-6">
            <div>
              <div className="text-[10px] uppercase tracking-[0.35em] text-amber-500/90 mb-3 font-medium">Private Cellar</div>
              <h1 className="font-display text-4xl md:text-5xl tracking-tight font-bold">Danny&rsquo;s Collection</h1>
              <p className="text-stone-400 mt-2 text-sm num">{pluralize(bottles.length, 'bottle', 'bottles')}</p>
            </div>
            <div className="hidden sm:block text-right">
              <div className="font-display text-5xl num text-amber-500 leading-none font-bold">{bottles.length}</div>
      ...
      <div className="sticky top-0 z-20 border-b border-stone-300/70" style={{ backgroundColor: 'rgba(244, 239, 230, 0.95)', backdropFilter: 'blur(8px)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3.5 flex gap-2.5 items-center">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-3 flex flex-wrap gap-2 items-center text-sm">
...
function BottleRow({ bottle, onEdit }) {
  ...
  // Order: memo first (most identifying info), then dates.
  // This way the memo always shows even when the line truncates.
  const subParts = [];
  if (memoOneLine) {
    subParts.push({ key: 'memo', text: memoOneLine, cls: 'text-stone-700' });
  }
  if (bottle.openedAt) {
  ...
  return (
    <li>
      <button
        onClick={onEdit}
        className="w-full text-left px-4 sm:px-5 py-4 hover:bg-stone-50 active:bg-stone-100 transition-colors flex items-start gap-3 sm:gap-4 group"
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-2 sm:gap-3">
            <h3 className={`font-display text-[17px] leading-tight truncate font-semibold tracking-tight ${
          ...
          {subParts.length > 0 && (
            <div className="text-[13px] text-stone-600 mt-1 truncate">
              {subParts.map((part, i) => (
                <span key={part.key}>
